...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", port: "vercel-serverless" });
});
...
app.post("/api/gemini/scan-invoice", async (req, res) => {
  ...
  console.log("Request received to /api/gemini/scan-invoice");
...
3. IGNORE ALL OTHER COPIES: Do not process or extract items from pages marked as 'DUPLICATE', 'TRIPLICATE', 'QUADRAPLICATE', 'EXTRA COPY', 'TRANSPORT COPY', or 'OFFICE COPY'.
...
Return the data strictly as a JSON object matching the requested schema.`;
    ...
    console.error("[AI SCAN ERROR] Gemini route exception", err);
    res.status(500).json({
      success: false,
